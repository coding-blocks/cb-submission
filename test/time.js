...
describe('#Test timed Submission', function () {
  it('Tests N^2 submissions', function (done) {
    ...
    fs.readFile(__dirname+'/../assests/input0.txt',function(err, testcases){
    	fs.readFile(__dirname+'/../assests/output0.txt',function(err, expected) {
        	const test_count = 1
		testcases = String(testcases);
		expected = String(expected);
		judge("cpp", source, test_count, [testcases], [expected], true, (body) => {
      			util.assertResult(body, "success", ["timelimit"], [""],false)
      			done()
    		});
       })
    });
 ...
 fs.readFile(__dirname+'/../assests/input0.txt',function(err, testcases){
	 fs.readFile(__dirname+'/../assests/output0.txt',function(err, expected) {
		 ...
		 expected = String(expected);
    		 judge("cpp", source, test_count, [testcases],[expected], true, (body) => {
      			util.assertResult(body, "success", ["correct"], [expected],false)
      			done()
    		});
	})
	});
